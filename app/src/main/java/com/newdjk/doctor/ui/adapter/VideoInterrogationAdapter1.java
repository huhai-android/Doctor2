    ...
    protected void convert(BaseViewHolder helper, final AllRecordForDoctorEntity item) {
       try {
           //  DoctorPatientRelationEntity DoctorPatientRelationEntity = item.getDoctorPatientRelation();
            Log.i("video", "AllRecordForDoctorEntity=" + item.toString());
            ...
            if (mType==3){
                if (!TextUtils.isEmpty(item.getEndTime())){
                    String endtime = item.getEndTime().substring(0,16);
                    if (!TextUtils.isEmpty(endtime) ) {

                        helper.setText(R.id.create_time,endtime);
                    }
                    else {
                ...
                if (!TextUtils.isEmpty(item.getPayTime())){
                    String createTime = item.getPayTime().substring(0,16);
            ...
            String date = item.getReExaminationDate();
            String day = date.substring(0,date.indexOf(" "));
            String startTime1 = item.getReExaminationStartTime();
            String startTime = startTime1.substring(0,startTime1.lastIndexOf(":"));
            String endTime1 = item.getReExaminationEndTime();
            String endTime =  endTime1.substring(0,endTime1.lastIndexOf(":"));
            helper.setText(R.id.appointment_time,day+"  "+startTime+"-"+endTime);
        } catch (Exception e) {
