...
public class AllApplicationAdapter extends BaseQuickAdapter<AppLicationEntity, BaseViewHolder> {
    ...
    public AllApplicationAdapter(@Nullable List<AppLicationEntity> data) {
        super(R.layout.application_item, data);
    ...
    protected void convert(BaseViewHolder helper, final AppLicationEntity item) {
        ...
        if (type.equals("编辑")){
            helper.getView(R.id.im_edit).setVisibility(View.GONE);
            if (item.isIsselect()) {
                helper.setImageResource(R.id.im_edit, R.drawable.duihao);

            } else {
                helper.setImageResource(R.id.im_edit, R.drawable.jiahao);
            }
