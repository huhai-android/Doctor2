...
public class AllDoctorDataAdapter extends BaseQuickAdapter<AllDoctorCheckEntity.ReturnDataBean, BaseViewHolder> {
    ...
    public AllDoctorDataAdapter(@Nullable List<AllDoctorCheckEntity.ReturnDataBean> data) {
    ...
    protected void convert(BaseViewHolder helper, final AllDoctorCheckEntity.ReturnDataBean item) {
        ...
        try {
            ...
            try {
                helper.setText(R.id.create_time, TimeUtil.showTime(item.getReplytime(), "yyyy-MM-dd HH:mm:ss"));
            } catch (Exception e) {
                helper.setText(R.id.create_time, item.getReplytime());
            }
