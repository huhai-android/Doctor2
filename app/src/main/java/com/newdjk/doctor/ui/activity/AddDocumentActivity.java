...
//        mMedicalView.show();
        ...
        mMyDiagnoseAdapter = new AddDocumentAdapter(mList);
    ...
    private SwipeMenuCreator swipeMenuCreator = new SwipeMenuCreator() {
        ...
        public void onCreateMenu(SwipeMenu swipeLeftMenu, SwipeMenu swipeRightMenu, int position) {
            ...
            if (viewType!=0){
                SwipeMenuItem deleteItem = new SwipeMenuItem(AddDocumentActivity.this).setBackground(R.drawable.selector_red)
    ...
    protected void initListener() {
        ...
        mMyDiagnoseAdapter.setOnDocumentAddListener(new AddDocumentAdapter.onDocumentAddListener() {
            @Override
    ...
    private void getMedicalItemsByMedicalRecordId() {
        ...
        String url = HttpUrl.GetMedicalItemsByMedicalRecordId + "?MedicalRecordId=" + mMedicalRecordId;
        ...
        mMyOkhttp.get().url(url).headers(headMap).tag(this).enqueue(new GsonResponseHandler<ResponseEntity<MedicalItemsEntity>>() {
            @Override
            public void onSuccess(int statusCode, ResponseEntity<MedicalItemsEntity> entity) {
                //    mConsultMessageAdapter.setDatalist( entity.getData());
                if (entity.getCode() == 0) {
                    mList.clear();
                    MedicalItemsEntity medicalItemsEntity = entity.getData();
                    mList.addAll(medicalItemsEntity.getUnDefalutMedicals());
                    mList.addAll(mList.size(), medicalItemsEntity.getDefinedMedicals());
    ...
    private void SendNeedSupplyMedical() {
        ...
        headMap.put("Authorization", SpUtils.getString(Contants.Token));
        // String url = "http://172.18.30.4/NetHospSecondDiagnosisAPI/MedicalService/QueryDoctorDiseases?DrId="+SpUtils.getInt(Contants.Id,0);
        mMyOkhttp.post().url(HttpUrl.SendNeedSupplyMedical).headers(headMap).jsonParams(json).tag(this).enqueue(new GsonResponseHandler<Entity>() {
            @Override
    ...
    private void drAddDefinedMedical(String medicalName, String medicalDesc) {
        ...
        headMap.put("Authorization", SpUtils.getString(Contants.Token));
        // String url = "http://172.18.30.4/NetHospSecondDiagnosisAPI/MedicalService/QueryDoctorDiseases?DrId="+SpUtils.getInt(Contants.Id,0);
        mMyOkhttp.post().url(HttpUrl.DrAddDefinedMedical).headers(headMap).params(map).tag(this).enqueue(new GsonResponseHandler<ResponseEntity<MedicalItemsEntity.MedicalsBean>>() {
            @Override
            public void onSuccess(int statusCode, ResponseEntity<MedicalItemsEntity.MedicalsBean> entity) {
                //    mConsultMessageAdapter.setDatalist( entity.getData());
                if (entity.getCode() == 0) {
                    MedicalItemsEntity.MedicalsBean medicalsBean = entity.getData();
    ...
    private void deleteDefinedMedical(final int position, DeleteMedicaEntity deleteMedicaEntity) {
        ...
        headMap.put("Authorization", SpUtils.getString(Contants.Token));
        // String url = "http://172.18.30.4/NetHospSecondDiagnosisAPI/MedicalService/QueryDoctorDiseases?DrId="+SpUtils.getInt(Contants.Id,0);
        mMyOkhttp.post().url(HttpUrl.DeleteDefinedMedical).headers(headMap).jsonParams(json).tag(this).enqueue(new GsonResponseHandler<ResponseEntity>() {
            @Override
