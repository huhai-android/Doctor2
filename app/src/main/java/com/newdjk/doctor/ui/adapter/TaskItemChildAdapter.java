...
public class TaskItemChildAdapter extends BaseQuickAdapter<TaskEntity.DataBean.DoctorOrderMergesBean, BaseViewHolder> {
    ...
    public TaskItemChildAdapter(@Nullable List<TaskEntity.DataBean.DoctorOrderMergesBean> data) {
    ...
    @Override
    protected void convert(final BaseViewHolder helper, final TaskEntity.DataBean.DoctorOrderMergesBean item) {
        ...
        if (item.getServiceType() == 19) {
            helper.setVisible(R.id.lv_desc, false);


        } else {
            helper.setVisible(R.id.lv_desc, true);
