...
public class ConsultMessageAdapter1 extends BaseQuickAdapter<AllRecordForDoctorEntity, BaseViewHolder> {
    ...
    @Override
    protected void convert(final BaseViewHolder helper, final AllRecordForDoctorEntity item) {
        long unReadNum = item.getUnReadNum();
        Log.i("ConsultMessage", "AllRecordForDoctorEntity" + item.toString());
        try {
            if (unReadNum > 0) {
            ...
            if (mType == 3) {
                if (!TextUtils.isEmpty(item.getEndTime())) {
                    String time = item.getEndTime().substring(0, 16);
                    helper.setText(R.id.create_time, time);
                }

                //状态（0：待接诊，1：已接诊/接诊中，2：已退诊，3：已回复-暂时没用，4：超时未接诊，5：已结束）
                if (item.getStatus()==2){
                ...
                if (!TextUtils.isEmpty(item.getPayTime())) {
                    String time = item.getPayTime().substring(0, 16);
                    helper.setText(R.id.create_time, time);
                }
        ...
        helper.getView(R.id.consult_message_item).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (mType == 3) {  //已完成
                    ...
                    json = mGson.toJson(prescriptionMessageEntity);
                    Log.i("zdp", "json=" + json);
                    SpUtils.put(Contants.patientName, item.getPatientName());
                    SpUtils.put(Contants.patientID, item.getPatientId());
